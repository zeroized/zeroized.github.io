...
在这个外积的展开形式里，我们能看到$v_i$和$v_j$的每一个维度都进行了交叉，是一种bit-wise的交叉（笔者觉得应该叫dimension/dim-wise），那么在最后使用MLP进行每个维度的权重计算时，就能产生更加细粒度的结果。笔者认为这一创新给后续的DCN、xDeepFM这些模型都提供了很好的思路（实际上并没有引用），虽然PNN的实现代码是笔者觉得最麻烦的代码之一（一方面很难直接用矩阵计算得出外积结果Batch * D1 * N * N的四维矩阵；另一方面，作者在代码里给出了外积的形式可以是矩阵、向量和数值，但论文里只说明了矩阵，笔者难以从论文中推导出向量和数值是怎么计算出来的，只能通过详细推导矩阵计算过程得出）。


#### Wide & Deep结构范式

Wide & Deep Network其实本身并不是一个特别好的模型（但他有谷歌亲爸爸光环加成啊）。需要手动指定cross要组合的列，有点回到了LR、在深度模型里“人工学习”的味道了。但Wide & Deep Network的贡献不在于本身模型好不好，而是它提出了一种很重要的模型结构范式：Deep和Cross**分开做**。
...
2020/06/30：修改PNN部分对“启发后续DCN和xDeepFM”的描述

2020/06/20：修改FNN的全称错误
